<script>
    ...
    async function eliminarMensajePrivado(idMensaje) {
        try {
            // ✅ Método de eliminación sin cambios (ya era único)
    ...
    async function cargarMensajes(inicial = false) {
        ...
        cargando = true;
        try {
            // ✅ Cambiado: obtener → obtenerMensajesPrivados
            const url = `${baseUrl}?c=mensajes&a=obtenerMensajesPrivados&destinatario_id=${destinatarioId}&last_id=${ultimoId}`;
    ...
    async function enviarMensaje() {
        ...
        try {
            // ✅ Cambiado: enviar → enviarMensajePrivado
            const resp = await fetch(`${baseUrl}?c=mensajes&a=enviarMensajePrivado`, {
                method: 'POST',
                body: formData
            });
            const data = await resp.json();
