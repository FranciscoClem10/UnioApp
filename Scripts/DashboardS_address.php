<script>
	...
	const addressCache = {};
	// Coordenadas que ya están en cola o en proceso, para no duplicar peticiones
	const pendingCoords = new Set();

	// Cola para peticiones con retardo (máx 1 petición por segundo)
	let addressQueue = [];
	...
	function renderLista() {
		...
		container.innerHTML = filtradas.map(act => {
			const coordKey = `${act.latitud},${act.longitud}`;
			const direccionHtml = addressCache[coordKey]
				? escapeHtml(addressCache[coordKey])
				: `<span class="dir-cargando" data-lat="${act.latitud}" data-lng="${act.longitud}">Cargando dirección...</span>`;
