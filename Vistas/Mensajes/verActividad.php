...
<head>
    <meta charset="UTF-8">
    <title>Chat de actividad - UnioApp</title>
    <style>
...
<body>
<div class="container">
    <h1>Chat de actividad: <?= htmlspecialchars($actividad['nombre']) ?></h1>
    ...
    <div id="mensajes">
        <?php foreach ($mensajes as $msg): ?>
            <div class="mensaje <?= $msg['id_usuario'] == $_SESSION['usuario_id'] ? 'mensaje-propio' : '' ?>">
                <div class="nombre">
                    <?= htmlspecialchars($msg['nombre_completo']) ?>
                    <?php if ($msg['id_usuario'] == $_SESSION['usuario_id']): ?>
                        <a href="<?= BASE_URL ?>?c=mensajes&a=eliminar&tipo=actividad&id=<?= $msg['id_mensaje'] ?>&id_act=<?= $actividad['id_actividad'] ?>" class="eliminar" onclick="return confirm('¿Eliminar este mensaje?')">🗑️</a>
                    <?php endif; ?>
                </div>
                <div><?= nl2br(htmlspecialchars($msg['contenido'])) ?></div>
                <div class="fecha"><?= $msg['fecha_envio'] ?></div>
            </div>
        <?php endforeach; ?>
    </div>
