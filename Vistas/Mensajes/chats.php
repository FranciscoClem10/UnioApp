...
<div class="mb-6 text-center md:text-left">
    ...
    <div class="flex flex-col flex-1 bg-background/80 backdrop-blur-sm rounded-2xl shadow-sm border border-outline-variant/20 overflow-hidden">
        <!-- Cabecera -->
        <div class="px-4 md:px-6 pt-4 md:pt-6 pb-2 md:pb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shrink-0"></div>
        ...
        <div id="listaConversaciones" class="flex-1 overflow-y-auto divide-y divide-outline-variant/20 px-2 md:px-4">
            <?php if (!empty($conversacionesActividad)): ?>
                ...
                <?php foreach ($conversacionesActividad as $c): ?>
                    <a href="<?= BASE_URL ?>?c=mensajesGrupo&a=verActividad&id=<?= $c['id_actividad'] ?>" class="conversation-item flex items-center gap-3 p-3 md:p-4 hover:bg-primary/10 rounded-xl transition-all">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center justify-between gap-2 mt-0.5">
                                ...
                                <?php if (isset($c['no_leidos']) && $c['no_leidos'] > 0): ?>
                                    <span class="bg-error text-on-error text-[11px] font-bold px-2 py-0.5 rounded-full"><?= $c['no_leidos'] > 9 ? '9+' : $c['no_leidos'] ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </a>
            ...
            <?php if (!empty($conversacionesPrivadas)): ?>
                ...
                <?php foreach ($conversacionesPrivadas as $c): ?>
                    <a href="<?= BASE_URL ?>?c=mensajes&a=verPrivado&id=<?= $c['id_usuario'] ?>" class="conversation-item flex items-center gap-3 p-3 md:p-4 hover:bg-primary/10 rounded-xl transition-all">
                        <div class="relative flex-shrink-0">
                            <?php if (!empty($c['foto_base64'])): ?>
                            ...
                            <?php if (isset($c['online']) && $c['online']): ?>
                                <span class="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-surface rounded-full"></span>
                            <?php endif; ?>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-baseline gap-2">
...
<div id="modalNuevoChat" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden transition-all">
    <div class="bg-surface rounded-2xl w-full max-w-md mx-4 shadow-xl transform transition-all border border-outline-variant/20">
        <div class="flex justify-between items-center p-5 border-b border-outline-variant/20">
                ...
                <div class="divide-y divide-outline-variant/20">
                    <?php foreach ($amigos as $amigo): ?>
                        <a href="<?= BASE_URL ?>?c=mensajes&a=verPrivado&id=<?= $amigo['id_usuario'] ?>" 
                           class="flex items-center gap-3 p-3 hover:bg-primary/10 rounded-xl transition">
                            <div class="relative flex-shrink-0">
...
<!-- Botón flotante + -->
<button id="btnNuevoChat" class="fixed bottom-24 right-4 md:bottom-6 md:right-6 z-50 w-14 h-14 bg-primary text-on-primary rounded-full shadow-lg flex items-center justify-center hover:bg-primary-dim transition active:scale-95">
