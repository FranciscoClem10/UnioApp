...
<nav class="md:hidden fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pt-3 pb-6 h-20 bg-white/70 backdrop-blur-xl border-t border-outline-variant/20 shadow-[0_-8px_32px_rgba(45,47,47,0.06)] z-50 rounded-t-2xl">
    ...
    <a href="<?= BASE_URL ?>?c=dashboard"
       class="flex flex-col items-center justify-center <?= $controlador === 'dashboard' ? 'text-primary' : 'text-on-surface opacity-60' ?>">
    ...
    <a href="<?= BASE_URL ?>?c=actividad&a=crear"
       class="flex flex-col items-center justify-center <?= ($controlador === 'actividad' && $accion === 'crear') ? 'text-primary' : 'text-on-surface opacity-60' ?>">
