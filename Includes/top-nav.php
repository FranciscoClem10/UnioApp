...
<header class="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-xl shadow-[0_8px_32px_rgba(45,47,47,0.06)] h-16 px-4 md:px-8 flex justify-between items-center">
    <div class="flex items-center gap-4 md:gap-8">
        ...
        <nav class="hidden md:flex gap-6">
            ...
            <!-- Mensajes privados -->
            <a class="p-2 rounded-full transition-transform active:scale-95 inline-block <?= ($_GET['c'] ?? '') == 'mensajes' ? 'text-primary font-bold' : 'text-slate-500 font-medium' ?>" href="<?= BASE_URL ?>?c=mensajes&a=chats">Mensajes privados</a>
            
            <!-- Mensajes de grupo -->
            <a class="p-2 rounded-full transition-transform active:scale-95 inline-block <?= ($_GET['c'] ?? '') == 'mensajesGrupo' ? 'text-primary font-bold' : 'text-slate-500 font-medium' ?>" href="<?= BASE_URL ?>?c=mensajesGrupo&a=chats">Mensajes de grupo</a>
        </nav>
    </div>
